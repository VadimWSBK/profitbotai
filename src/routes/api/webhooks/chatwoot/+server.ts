 ...
 * Inbound webhook for Chatwoot Agent Bot. Chatwoot sends conversation events (e.g. message_created)
 * to this URL. When the user sends an incoming message, we run the configured Profitbot agent
 * and post the reply back via Chatwoot's Messages API.
 *
 * Configure in Chatwoot: Settings → Bots → Add Bot → Webhook URL:
 *   https://YOUR-PROFITBOT-DOMAIN/api/webhooks/chatwoot
 *
 * Required env:
 *   CHATWOOT_AGENT_ID       – Profitbot agent UUID (uses its role, tone, instructions, LLM)
 *   CHATWOOT_BOT_ACCESS_TOKEN – Bot token from Chatwoot (create bot → copy token from API/console)
...
export const POST: RequestHandler = async (event) => {
	const agentId = (env.CHATWOOT_AGENT_ID ?? '').trim();
	...
	if (!agentId || !botToken || !baseUrl) {
		console.error('[webhooks/chatwoot] Missing env: CHATWOOT_AGENT_ID, CHATWOOT_BOT_ACCESS_TOKEN, or CHATWOOT_BASE_URL');
