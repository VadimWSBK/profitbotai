-- Switch all vector embeddings to 1536 dimensions for n8n compatibility.
-- Using 1536 instead of 3072 because HNSW indexes support max 2000 dimensions.
-- Configure n8n to use output_dimensionality: 1536, or truncate 3072-dim embeddings to 1536.
...
alter table public.widget_documents add column embedding vector(1536);
...
alter table public.agent_documents add column embedding vector(1536);
...
alter table public.agent_rules add column embedding vector(1536);
...
comment on function public.match_documents(jsonb, int, vector) is 'Vector similarity search on widget_documents (1536 dims - compatible with n8n gemini-embedding-001 using output_dimensionality: 1536)';
comment on function public.match_agent_rules_documents(jsonb, int, vector) is 'Vector similarity search on agent_rules (1536 dims - compatible with n8n gemini-embedding-001 using output_dimensionality: 1536)';
