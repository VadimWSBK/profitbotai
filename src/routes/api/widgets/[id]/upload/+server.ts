...
/**
 * POST /api/widgets/[id]/upload – upload an icon/image for this widget.
 * Body: multipart/form-data with field "file" (and optional "field": "bubble" | "header" | "avatar").
 * Returns { url: string } (public URL) or error.
 */
export const POST: RequestHandler = async (event) => {
	const widgetId = event.params.id;
	if (!widgetId) return json({ error: 'Missing widget id' }, { status: 400 });
	if (!event.locals.user) return json({ error: 'Unauthorized' }, { status: 401 });
	// Only admins can upload (widgets are admin-only)
	...
	const path = `${widgetId}/${crypto.randomUUID()}.${safeExt}`;
	...
	try {
		...
		console.error('POST /api/widgets/[id]/upload:', e);
