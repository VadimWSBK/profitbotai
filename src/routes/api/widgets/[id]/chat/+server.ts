				...
				if (wantsDiy) {
					...
					if (shopifyConnected && roofSqm !== null && roofSqm >= 1) {
						triggerResult = {
							...
							webhookResult: `The customer wants DIY and we have their roof size (${roofSqm} m²). Call calculate_bucket_breakdown(roof_size_sqm: ${roofSqm})—it returns breakdown and checkout link. List products as BULLET POINTS (• N × Product on separate lines), then total, then the full checkout preview block. Do NOT use generate_quote or create a PDF for DIY.`
						};
					} else {
						triggerResult = {
							triggerId: triggerResult!.triggerId,
				...
				if (
					...
					if (shopifyConnected) {
						triggerResult = {
							triggerId: triggerResult!.triggerId,
							triggerName: triggerResult!.triggerName,
							webhookResult: `The customer is asking for a price and we have their roof size (${roofSqmForDiy} m²). Call calculate_bucket_breakdown(roof_size_sqm: ${roofSqmForDiy})—it returns breakdown and checkout link. List products as BULLET POINTS (• N × Product on separate lines), then total, then the full checkout preview block. Do NOT use generate_quote or create a PDF. If they want to proceed to checkout, ask for their name and email.`
						};
						console.log(
