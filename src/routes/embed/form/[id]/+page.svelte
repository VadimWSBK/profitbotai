...
<div class="min-h-[400px] w-full max-w-lg mx-auto p-6 bg-white rounded-2xl shadow-lg" style="--primary: {primaryColor}">
	{#if pdfUrl}
	...
	{:else}
		...
		{#if currentStepData?.description}
			<p class="text-sm text-gray-500 mb-6">{currentStepData.description}</p>
		{:else}
			<p class="text-sm text-gray-500 mb-6">Step {currentStep + 1} of {totalSteps}</p>
		{/if}
