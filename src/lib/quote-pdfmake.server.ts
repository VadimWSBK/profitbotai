...
export function buildQuoteDocDefinition(settings: QuoteSettings, payload: QuotePayload): DocDef {
			...
			columns: [
				{
					width: '*',
					stack: [
						...(settings.logo_base64 ? [{ image: settings.logo_base64, width: Math.max(20, Math.min(400, Number(settings.logo_size) || 120)), margin: [0, 0, 0, 8] }] : []),
	...
	if ((bankDetails as QuoteBankDetails)?.name || settings.barcode_url) {
		...
		if (settings.barcode_base64) {
			qrStack.push(
				...
				{ image: settings.barcode_base64, width: Math.max(20, Math.min(300, Number(settings.qr_size) || 80)), margin: [0, 4, 0, 0] }
			);
	...
	const footerContent = () => [
		...
		{
			columns: [
				{ text: 'Thank you for your interest!', fontSize: 9 },
				{ text: `Generated by ${company.name ?? ''}`, fontSize: 9, alignment: 'right' }
			]
		}
	];
