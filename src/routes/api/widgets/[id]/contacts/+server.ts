 ...
 * Body: { conversationId: string, name?: string, email?: string, phone?: string, address?: string }
...
export const PATCH: RequestHandler = async (event) => {
	...
	if (Object.keys(updates).length === 0) {
		return json({ error: 'No fields to update (name, email, phone, address)' }, { status: 400 });
	}
